...
func TestSearchRuntime(t *testing.T) {
	...
	response, err := searchv1.NewSearchServiceClient(conn).SearchProperties(ctx, &searchv1.SearchPropertiesRequest{Keyword: "foundationfixture", Page: 1, Limit: 10})
	if err != nil {
		t.Fatal(err)
	}
	if response.Page != 1 || response.Limit != 10 || response.Total != 1 || response.TotalPages != 1 || len(response.Items) != 1 || response.Items[0].Id != "foundation-1" {
